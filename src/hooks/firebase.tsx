...
const firebaseConfig = {
  apiKey: "AIzaSyCUUOrPA_DjJwX6d99c4ZDs8nu7V9flsz8",
  authDomain: "aurealone-fbca9.firebaseapp.com",
  projectId: "aurealone-fbca9",
  storageBucket: "aurealone-fbca9.firebasestorage.app",
  messagingSenderId: "888492962739",
  appId: "1:888492962739:web:bcb116fa784ebe377e11c6",
  measurementId: "G-MBNBH8Y2W7",
};
...
interface EmailSignInPayload {
  email: string;
  password: string;
  type: AUTH_METHOD.EMAIL_REGISTER | AUTH_METHOD.EMAIL_SIGNIN;
}
...
export const useFirebaseSignIn = () => {
  return useMutation({
    mutationFn: async (payload: EmailSignInPayload | GoogleSignInPayload) => {
      if (payload.type === AUTH_METHOD.EMAIL_REGISTER) {
        return await createUserWithEmailAndPassword(
          auth,
          payload.email,
          payload.password
        );
      } else if (payload.type === AUTH_METHOD.EMAIL_SIGNIN) {
        return await signInWithEmailAndPassword(
          auth,
          payload.email,
          payload.password
        );
      } else if (payload.type === AUTH_METHOD.GOOGLE_SIGNIN) {
...
export const withoutAuth = <P extends object>(
  ...
  const WithAuth: FC<P> = (props) => {
    ...
    if (user === null) return <WrappedComponent {...props} />;
    return (
      <Center my="14">
        <Spinner size="lg" thickness="3px" color="white" />
      </Center>
...
export const AUTH_ERRORS: Record<any, string> = {
  "auth/wrong-password":
